...
class Model:
    ...
    def __init__(self):
        ...
        path = 'D:\Depression Detection Major Project\Depression_Detection_Using_Machine_Learning-main\dataset\depression_Dataset.csv'
        df = pd.read_csv(path)
    ...
    def decisionTree_classifier(self):
        ...
        classifier = DecisionTreeClassifier(criterion="entropy", max_depth=10, splitter='random', min_samples_split=50)
        return classifier.fit(self.x_train,self.y_train)
    ...
    def accuracy(self,model):
        ...
        f1_meaure = (precision * recall) / (precision + recall)
        print(f"{self.name} has accuracy of {accuracy *100} % ")
        print(f"{self.name} has error rate of {error_rt * 100} %")
        print(f"{self.name} has precision of {precision}")
        print(f"{self.name} has recall of {recall}")
        print(f"{self.name} has f1 measure of {f1_meaure}")
        # cm_display = metrics.ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=[0, 1, 2, 3, 4])
        # cm_display.plot()
        # plt.show()
